...
(defn seat->id
  [seat]
  (-> seat
      (clojure.string/replace #"." {"F" "0", "L" "0", "B" "1", "R" "1"})
      (Integer/parseInt 2)))

;; Overkill, i could've just do it the simple way with loop+recur
(defn juxt-reduce
...
(defn day5
  [input]
  (let [[min-id max-id xor-id] (->> (clojure.string/split-lines input)
                                    (map seat->id)
                                    (juxt-reduce [min max bit-xor]))]
    ;; part1
    (println max-id)
    ;; part2
